...
# Optional: where an ecosystem's packages live, when the repo does not declare that itself.
# Only needed by a repo with no root package.json to put `workspaces` in — see the npm adapter.
[discovery]
npm = ["packages/*", "types"]

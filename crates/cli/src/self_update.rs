...
pub fn run() -> Result<()> {
    ...
    let latest_tag = json["tag_name"].as_str().context("missing tag_name in github response")?;
    let latest_version = latest_tag.trim_start_matches('v');

    if current_version == latest_version {
        println!("You are already using the latest version (v{}).", current_version);
        return Ok(());
    }

    println!("Updating otf-release from v{} to v{}...", current_version, latest_version);
    ...
    if status.success() {
        println!("Successfully updated otf-release from v{} to v{}!", current_version, latest_version);
    } else {

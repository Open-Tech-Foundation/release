...
- **update** — A brand new top-level subcommand that instantly regenerates `.github/workflows/release.yml` from an existing `release.toml` configuration, allowing you to easily adopt new CI pipeline features (like the new `check-release` job that guards expensive cross-compilation).
